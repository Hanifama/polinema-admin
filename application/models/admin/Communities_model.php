...
class Communities_model extends CI_Model
{
  ...
  public $v_fields=array('community_id', 'name', 'description', 'logo', 'community_categories.name');
	...
	function getList($table, $pagination=array()) 
  {
    ...
    $this->db->select("$table.*  , community_categories.name as category_id ");
    $this->db->from($table);

     $this->db->join("community_categories", "communities.category_id = community_categories.category_id", "left");     
  ...
  function getRow($table, $id) {
    $this->db->select('*');
    $query = $this->db->get_where($table, array('community_id' => $id));
    $data = $query->result();
