...
class Creation_model extends CI_Model
{
  ...
  public $v_fields=array('creation_id', 'title', 'meta_title', 'short_description', 'thumbnail', 'content', 'product.name', 'category', 'tag', 'is_featured', 'status');
	...
	function getList($table, $pagination=array()) 
  {
    ...
    $this->db->select("$table.*  , product.name as product_id ");
    $this->db->from($table);

     $this->db->join("product", "creation.product_id = product.product_id", "left");     

    $this->db->order_by("creation_id","desc");
    $query = $this->db->get();
  ...
  function getRow($table, $id) {
    $this->db->select('*');
    $query = $this->db->get_where($table, array('creation_id' => $id));
    $data = $query->result();
