...
class Discussion_comment_model extends CI_Model
{

  public $v_fields=array('comment_id', 'user_id', 'discussion.discus_id', 'created_dt', 'content', 'attachment', 'attachment_mime');
	...
	function getList($table, $pagination=array()) 
  {
    ...
    $this->db->select("$table.*  , discussion.discus_id as discus_id ");
    $this->db->from($table);

     $this->db->join("discussion", "discussion_comment.discus_id = discussion.discus_id", "left");     
  ...
  function getRow($table, $id) {
    $this->db->select('*');
    $query = $this->db->get_where($table, array('comment_id' => $id));
    $data = $query->result();
