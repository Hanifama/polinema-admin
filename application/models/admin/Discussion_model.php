...
class Discussion_model extends CI_Model
{
  ...
  public $v_fields=array('discus_id', 'title', 'content', 'attachment', 'attachment_mime', 'created_dt', 'users.name', 'view_cnt', 'like_cnt', 'comment_cnt', 'communities.name');
	...
	function getList($table, $pagination=array()) 
  {
    ...
    $this->db->select("$table.*  , users.name as user_id  , communities.name as community_id ");
    $this->db->from($table);

     $this->db->join("users", "discussion.user_id = users.user_id", "left");  $this->db->join("communities", "discussion.community_id = communities.community_id", "left");     

    $this->db->order_by("discus_id","desc");
    $query = $this->db->get();
  ...
  function getRow($table, $id) {
    $this->db->select('*');
    $query = $this->db->get_where($table, array('discus_id' => $id));
    $data = $query->result();
