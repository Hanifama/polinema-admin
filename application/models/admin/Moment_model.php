...
class Moment_model extends CI_Model
{
  ...
  public $v_fields=array('moment_id', 'title', 'meta_title', 'short_description', 'thumbnail', 'content', 'tag', 'is_featured', 'status');
	...
	function getList($table, $pagination=array()) 
  {
    ...
    $this->db->order_by("moment_id","desc");
    $query = $this->db->get();
  ...
  function getRow($table, $id) {
    $this->db->select('*');
    $query = $this->db->get_where($table, array('moment_id' => $id));
    $data = $query->result();
