...
class Post_comments_model extends CI_Model
{
  ...
  public $v_fields=array('comment_id', 'posts.post_id', 'users.name', 'content', 'created_dt');
	...
	function getList($table, $pagination=array()) 
  {
    ...
    $this->db->select("$table.*  , posts.post_id as post_id  , users.name as user_id ");
    $this->db->from($table);

     $this->db->join("posts", "post_comments.post_id = posts.post_id", "left");  $this->db->join("users", "post_comments.user_id = users.user_id", "left");     
