...
class Posts_model extends CI_Model
{
  ...
  public $v_fields=array('post_id', 'usersa.name', 'usersb.name', 'content', 'attachment', 'type', 'privacy', 'created_dt', 'replied');
	...
	function getList($table, $pagination=array()) 
  {
    ...
    $this->db->select("$table.*  , usersa.name as wall_owner_id  , usersb.name as author_id ");
    $this->db->from($table);

     $this->db->join("users as usersa", "posts.wall_owner_id = usersa.user_id", "left");  
     $this->db->join("users as usersb", "posts.author_id = usersb.user_id", "left");     
  ...
  function getRow($table, $id) {
    $this->db->select('*');
    $query = $this->db->get_where($table, array('post_id' => $id));
    $data = $query->result();
  ...
  function getCount($table, $key='', $value='') {
    ...
    $this->db->from($table);
     ...
     $this->db->join("users as usera", "posts.wall_owner_id = usera.user_id", "left");  $this->db->join("users as userb", "posts.author_id = userb.user_id", "left"); 
