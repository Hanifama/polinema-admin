...
class Programs_model extends CI_Model
{
  ...
  public $v_fields=array('program_id', 'departments.name', 'name', 'created_dt');
	...
	function getList($table, $pagination=array()) 
  {
    ...
    $this->db->select("$table.*  , departments.name as department_id ");
    $this->db->from($table);

     $this->db->join("departments", "programs.department_id = departments.department_id", "left");     
  ...
  function getRow($table, $id) {
    $this->db->select('*');
    $query = $this->db->get_where($table, array('program_id' => $id));
    $data = $query->result();
