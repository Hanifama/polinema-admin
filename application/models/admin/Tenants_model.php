...
class Tenants_model extends CI_Model
{
  ...
  public $v_fields=array('tenant_id', 'tenant_categories.name', 'name', 'banner', 'address', 'about', 'image_1', 'image_2', 'image_3', 'image_4', 'lat', 'lng', 'status');
	...
	function getList($table, $pagination=array()) 
  {
    ...
    $this->db->select("$table.*  , tenant_categories.name as tencat_id ");
    $this->db->from($table);

     $this->db->join("tenant_categories", "tenants.tencat_id = tenant_categories.tencat_id", "left");     
  ...
  function getRow($table, $id) {
    $this->db->select('*');
    $query = $this->db->get_where($table, array('tenant_id' => $id));
    $data = $query->result();
