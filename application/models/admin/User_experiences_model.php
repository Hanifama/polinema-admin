...
class User_experiences_model extends CI_Model
{

  public $v_fields=array('experience_id', 'users.name', 'company_name', 'company_category', 'position', 'start_year', 'end_year', 'description');
	...
	function getList($table, $pagination=array()) 
  {
    ...
    $this->db->select("$table.*  , users.name as user_id ");
    $this->db->from($table);

     $this->db->join("users", "user_experiences.user_id = users.user_id", "left");     
