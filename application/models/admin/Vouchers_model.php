...
class Vouchers_model extends CI_Model
{
  ...
  public $v_fields=array('voucher_id', 'tenants.name', 'title', 'description', 'banner_1', 'banner_2', 'banner_3', 'banner_4', 'discount_type', 'discount_value', 'minimum_amount', 'maximum_discount', 'start_dt', 'end_dt', 'is_claimed', 'quota', 'used', 'created_dt', 'status', 'voucher_categories.name');
	...
	function getList($table, $pagination=array()) 
  {
    ...
    $this->db->select("$table.*  , tenants.name as tenant_id  , voucher_categories.name as vocat_id ");
    $this->db->from($table);

     $this->db->join("tenants", "vouchers.tenant_id = tenants.tenant_id", "left");  $this->db->join("voucher_categories", "vouchers.vocat_id = voucher_categories.vocat_id", "left");     
  ...
  function getRow($table, $id) {
    $this->db->select('*');
    $query = $this->db->get_where($table, array('voucher_id' => $id));
    $data = $query->result();
