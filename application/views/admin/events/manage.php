   ...
   <div class="col-lg-12">
      <div class="ibox card mt-3 ">
         ...
         <div class="ibox-title card-header">
            ...
            <a href="<?php echo base_url(); ?>admin/events/add" class="btn btn-info m-2">ADD NEW</a>
            ...
            <form method="GET" action="<?php echo base_url().'admin/events/index'; ?>" class="form-inline ibox-content">
               <div class="form-group m-2">
                  <select name="searchBy" class="form-control">
                  <option value="event_name" <?php echo $searchBy=="event_name"?'selected="selected"':""; ?>>Event Name</option><option value="event_date" <?php echo $searchBy=="event_date"?'selected="selected"':""; ?>>Event Date</option><option value="location" <?php echo $searchBy=="location"?'selected="selected"':""; ?>>Location</option><option value="status" <?php echo $searchBy=="status"?'selected="selected"':""; ?>>Status</option>
                  </select>
               </div>
         ...
         <div class="ibox-content card-body">
            <div class="table table-responsive">
               <table class="table table-striped table-bordered table-hover Tax" style="font-size: 13px;" >
				...
				<?php $symbol = isset($_GET["sortBy"]) && $_GET["sortBy"]=="event_name"?"<i class='fa fa-sort-$sortSym' aria-hidden='true'></i>": "<i class='fa fa-sort' aria-hidden='true'></i>"; ?>

				<th> <a href="<?php echo $fields_links["event_name"]; ?>" class="link_css"> Name <?php echo $symbol ?></a></th>

						

				<?php $symbol = isset($_GET["sortBy"]) && $_GET["sortBy"]=="event_date"?"<i class='fa fa-sort-$sortSym' aria-hidden='true'></i>": "<i class='fa fa-sort' aria-hidden='true'></i>"; ?>

				<th> <a href="<?php echo $fields_links["event_date"]; ?>" class="link_css"> Date <?php echo $symbol ?></a></th>
				...
				<th> <a href="<?php echo $fields_links["created_dt"]; ?>" class="link_css"> Created <?php echo $symbol ?></a></th>
                  ...
                  <tbody>
                     ...
                     <?php 
                        ...
                        ?>
                     <tr  id="hide<?php echo $value->event_id; ?>" >
                     <th><input name='input' id='del' onclick="callme('show')"  type='checkbox' class='del' value='<?php echo $value->event_id; ?>'/></th>

                              

            <th><?php if(!empty($value->event_id)){ echo $count; $count++; }?></th>

                <th><?php if(!empty($value->event_name)){ echo $value->event_name; }?></th>

                <th><?php if(!empty($value->event_date)){ echo $value->event_date; }?></th>

                <th><?php if(!empty($value->location)){ echo $value->location; }?></th>

                <th><?php if(!empty($value->photo)){ ?> 

                        <img src="<?php echo $this->config->item('photo_url');?><?php echo $value->photo; ?>" alt="pic" width="50" height="50" />

                         <?php }?></th><th><?php if(!empty($value->description)){ echo $value->description; }?></th>
		   ...
		   <a href="<?php echo base_url()?>admin/events/view/<?php echo $value->event_id; ?>" class="btn btn-info" title="View">
            <i class="fa fa-eye"></i>
           </a>
           <a href="<?php echo base_url()?>admin/events/edit/<?php echo $value->event_id; ?>" class="btn btn-info" title="Edit">
            <i class="fa fa-edit"></i>
           </a>
           <a  title="Delete" data-toggle="modal" data-target="#commonDelete" class="btn btn-info" onclick="set_common_delete('<?php echo $value->event_id; ?>','events');">
...
<script type="text/javascript">
   ...
   function delRow()
   {
      ...
      if(confrm)
      {
         ...
         $.ajax({
         type:"POST",
         url:'<?php echo base_url()."admin/events/deleteAll"; ?>',
         data:{
            allIds : ids,
