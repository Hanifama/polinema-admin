   ...
   <div class="col-lg-12">
      <div class="ibox card mt-3 ">
         ...
         <div class="ibox-title card-header">
            ...
            <a href="<?php echo base_url(); ?>admin/tenant_categories/add" class="btn btn-info m-2">ADD NEW</a>

            <div class="form-group pull-right m-2">
               <a href="<?php echo $csvlink; ?>" class="btn btn-info">CSV</a>
            </div>

            <form method="GET" action="<?php echo base_url().'admin/tenant_categories/index'; ?>" class="form-inline ibox-content">
               <div class="form-group m-2">
                  <select name="searchBy" class="form-control">
                  <option value="name" <?php echo $searchBy=="name"?'selected="selected"':""; ?>>Name</option><option value="status" <?php echo $searchBy=="status"?'selected="selected"':""; ?>>Status</option>
                  </select>
               </div>
         ...
         <div class="ibox-content card-body">
            <div class="table table-responsive">
               <table class="table table-striped table-bordered table-hover Tax" style="font-size: 13px;" >
				...
				<?php $symbol = isset($_GET["sortBy"]) && $_GET["sortBy"]=="icon"?"<i class='fa fa-sort-$sortSym' aria-hidden='true'></i>": "<i class='fa fa-sort' aria-hidden='true'></i>"; ?>

				<th> <a href="<?php echo $fields_links["icon"]; ?>" class="link_css"> Icon <?php echo $symbol ?></a></th>
				...
				<th> <a href="<?php echo $fields_links["status"]; ?>" class="link_css"> Status <?php echo $symbol ?></a></th>
                  ...
                  <tbody>
                     ...
                     <?php 
                        ...
                        ?>
                     <tr  id="hide<?php echo $value->tencat_id; ?>" >
                     <th><input name='input' id='del' onclick="callme('show')"  type='checkbox' class='del' value='<?php echo $value->tencat_id; ?>'/></th>

                              

            <th><?php if(!empty($value->tencat_id)){ echo $count; $count++; }?></th>

                <th><?php if(!empty($value->name)){ echo $value->name; }?></th>

                <th><?php if(!empty($value->icon)){ ?> 

                        <img src="<?php echo $value->icon; ?>" alt="pic" width="50" height="50" />

                         <?php }?></th><th><?php if(!empty($value->status)){ echo $value->status; }?></th>

                <th class="action-width btn-group">

		   
           <a href="<?php echo base_url()?>admin/tenant_categories/edit/<?php echo $value->tencat_id; ?>" class="btn btn-info" title="Edit">
            <i class="fa fa-edit"></i>
           </a>
           <a  title="Delete" data-toggle="modal" data-target="#commonDelete" class="btn btn-info" onclick="set_common_delete('<?php echo $value->tencat_id; ?>','tenant_categories');">
...
<script type="text/javascript">
   ...
   function delRow()
   {
      ...
      if(confrm)
      {
         ...
         $.ajax({
         type:"POST",
         url:'<?php echo base_url()."admin/tenant_categories/deleteAll"; ?>',
         data:{
            allIds : ids,
